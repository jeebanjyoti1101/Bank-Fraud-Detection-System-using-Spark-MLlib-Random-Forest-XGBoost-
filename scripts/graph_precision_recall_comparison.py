"""
Precision-Recall Curve Comparison - All Models (Genuine Data)
Displays Precision-Recall curves for RF, XGBoost, LightGBM, and CatBoost using real models
"""
import pandas as pd
import numpy as np
...
print("=" * 70)
print("📈 Precision-Recall Curve Comparison - All Models (Genuine Data)")
print("=" * 70)
...
for idx, (name, model) in enumerate(models.items()):
    try:
        ...
        ax.plot(recall, precision, linewidth=3, label=f'{name} (AP = {avg_precision:.4f})', 
...
ax.axhline(y=baseline, color='k', linestyle='--', linewidth=2, 
           label=f'Baseline (AP = {baseline:.4f})', alpha=0.6)

# Styling
ax.set_xlabel('Recall', fontsize=14, fontweight='bold')
ax.set_ylabel('Precision', fontsize=14, fontweight='bold')
ax.set_title('Precision-Recall Curve Comparison - All Models', fontsize=16, fontweight='bold', pad=20)
...
print("\n" + "=" * 70)
print("✅ PRECISION-RECALL CURVE COMPARISON COMPLETE!")
print("=" * 70)
print(f"📊 Test Set Info:")
print(f"   Total samples: {len(y_test):,}")
print(f"   Fraud cases: {sum(y_test):,} ({sum(y_test)/len(y_test)*100:.2f}%)")
print(f"   Non-fraud cases: {len(y_test)-sum(y_test):,} ({(len(y_test)-sum(y_test))/len(y_test)*100:.2f}%)")
print("=" * 70)
